...
# 
# Umapathy Ravichandiran : updated IP address of Gateway Server
...
#
# Umapathy Ravichandiran: Updated IP addresses of the client-side network
#
...
#Umapathy Ravichandiran: Updated IP aliases for NATed services (this is the GW's ip on client network)
...
# Umapathy Ravichandiran: Updated client IP and Allow http traffic from client network to server network
$IPTABLES -A FORWARD -p icmp -o $Client_NET_IFACE -s 8.8.8.8 -j ACCEPT
$IPTABLES -A FORWARD -p icmp -s 10.2.0.6 -i $Client_NET_IFACE -d 8.8.8.8 -j ACCEPT
...
#
# Umapathy Ravichandiran: Updated input rules below to allow web traffic from client
#
$IPTABLES -A INPUT -p TCP --dport 80 -i $Client_NET_IFACE -s 10.0.2.6 -j ACCEPT
...
# Umapathy Ravichandiran: Updated IP address of client and fixed typo $IPTABLES -A OUTPUT -p icmp -j ACCEPT
$IPTABLES -A OUTPUT -p TCP --sport 80 -o $Client_NET_IFACE -d 10.0.2.6 -j ACCEPT
...
# Umapathy Ravichandiran: Updated to allow client node to access to all Internet using masquerade
